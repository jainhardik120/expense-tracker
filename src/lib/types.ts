...
import type { FormApi, ValidationError, FormState, AnyFormApi } from '@tanstack/form-core';
...
export type DynamicText = string | ((values: Record<string, unknown>) => string);
...
export interface ObjectConfig {
  ...
  fields: Array<{
    ...
    options?:
      ...
      | ((values: Record<string, unknown>) => string[] | Array<{ value: string; label: string }>);
    min?: number;
    max?: number;
    step?: number;
    [key: string]: unknown;
...
export interface FieldConfig {
  ...
  options?:
    ...
    | ((values: Record<string, unknown>) => string[] | { value: string; label: string }[]);
  min?: number;
  max?: number;
  step?: number;
  accept?: string;
