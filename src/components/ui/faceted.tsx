...
const useFacetedContext = (name: string) => {
...
const Faceted = <Multiple extends boolean>(props: FacetedProps<Multiple>) => {
  const {
    open: openProp,
...
const FacetedTrigger = (props: React.ComponentProps<typeof PopoverTrigger>) => {
...
const FacetedBadgeList = (props: FacetedBadgeListProps) => {
  const {
    options = [],
    max = 2,
...
const FacetedContent = (props: React.ComponentProps<typeof PopoverContent>) => {
...
const FacetedItem = (props: FacetedItemProps) => {
  ...
  const onItemSelect = React.useCallback(
    (currentValue: string) => {
      ...
      } else if (contextOnItemSelect !== undefined) {
        contextOnItemSelect(currentValue);
      }
    },
    [onSelect, contextOnItemSelect],
  );
